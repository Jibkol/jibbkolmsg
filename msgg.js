...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function sendPhotoToChat(imageUrl) {
        ...
        sortChats(); reorderChatDOM(); reorderChatDOM(); updateChatList();
    ...
    function simulateReply(chatId) {
        ...
        typingIndicator.innerHTML = `<p class="typing-dots"><span></span><span></span><span></span></p>`;
        msgContent.appendChild(typingIndicator);
        scrollToBottom(chatElement);

        // Random typing duration (1.2s - 2.5s)
        const typingDuration = 1200 + Math.random() * 1300;
        ...
        setTimeout(() => {
            ...
            // Update chat meta and move it to top
            const chat = appState.chats.find(c => c.id === chatId);
            if (chat) {
            ...
            updateMessageDisplay(chatId);
            sortChats(); reorderChatDOM(); reorderChatDOM(); // reorder DOM so last messaged chat moves to top
            updateChatList();
    ...
    // --- typing indicator CSS injected via JS for the dots animation ---
    (function injectTypingStyles(){
        if (document.getElementById('typing-indicator-styles')) return;
        const s = document.createElement('style');
        s.id = 'typing-indicator-styles';
        s.textContent = `
            .typing-dots { display:flex; gap:6px; align-items:center; padding:6px 10px; }
            .typing-dots span { width:8px; height:8px; border-radius:50%; background:rgba(255,255,255,0.25); opacity:0.6; transform:translateY(0); animation:typingBounce 1s infinite; }
            .typing-dots span:nth-child(2) { animation-delay:0.2s; }
            .typing-dots span:nth-child(3) { animation-delay:0.4s; }
            @keyframes typingBounce { 0%{ transform:translateY(0); opacity:0.6 } 50%{ transform:translateY(-6px); opacity:1 } 100%{ transform:translateY(0); opacity:0.6 } }
            .typing-indicator { max-width:60%; border-radius:10px; background:var(--wa-bubble-incoming-dark); margin:8px 0; padding:6px 10px; }
        `;
        document.head.appendChild(s);
    })();

    function updateMessageDisplayfunction updateMessageDisplay(chatId) {
        const chatElement = document.getElementById(chatId);
        if (!chatElement) {
    ...
    // Reorder chat DOM elements to match appState.chats order (most recent first)
    function reorderChatDOM() {
        try {
            const chatsContainer = document.getElementById('chats-list');
            if (!chatsContainer) return;
            // Build a map of chatId => element
            const elements = Array.from(chatsContainer.querySelectorAll('.chat'));
            const map = new Map();
            elements.forEach(el => map.set(el.getAttribute('data-chat'), el));
            // Clear container and re-insert in order from appState.chats
            appState.chats.forEach(chat => {
                const el = map.get(chat.id);
                if (el) {
                    chatsContainer.appendChild(el);
                }
            });
        } catch (e) { console.error('reorderChatDOM error', e); }
    }
...
overlay.addEventListener("click" ,()=>{
    ...
    // Make input bars sticky/fixed at bottom so they sit above mobile keyboard when focused
    (function makeInputAreaSticky(){
        function applySticky() {
            document.querySelectorAll('.jibapp-message').forEach(chat => {
                const inputArea = chat.querySelector('.msg-input-area');
                if (!inputArea) return;
                // Default sticky behavior
                inputArea.style.position = 'sticky';
                inputArea.style.bottom = '0px';
                inputArea.style.zIndex = '900';
                inputArea.style.background = getComputedStyle(inputArea).backgroundColor || 'transparent';
                // On focus, make it fixed to better handle mobile keyboards
                const input = inputArea.querySelector('.msg-input');
                if (input && !input.getAttribute('data-sticky-attached')) {
                    ...
                    input.setAttribute('data-sticky-attached','1');
                }
            });
        }
        // apply on load and whenever a new chat element is added
        applySticky();
        new MutationObserver(applySticky).observe(document.getElementById('chat-inner') || document.body, {childList:true, subtree:true});
    })();
});